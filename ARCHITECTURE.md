...
| `tools/placecheck.py` | every state and city row resolves, in four spellings, to its own zone under its own name, with nothing asked earlier shadowing it; the names left out stay out; with `--geonames`, every zone against GeoNames | without `--geonames`, whether a zone is *right* — a wrong zone written into both ports passes every other harness too |
