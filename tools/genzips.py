#!/usr/bin/env python3
"""Regenerate the US ZIP-prefix time zone table in ziptz/ziptz.go and ziptz/ziptz.py.

Development tool; the ziptz libraries themselves need nothing but the standard
library. It writes both files in one pass, which is what keeps the two embedded
...
def zip_zone_targets(root):
    """The letter -> zone map the libraries use, read from ziptz/ziptz.py.
    ...
    body = re.search(
        r"ZONES = \{(.*?)\}",
        root.joinpath("ziptz", "ziptz.py").read_text(encoding="utf-8"),
        re.S,
    )
    if not body:
        sys.exit("genzips: cannot find ZONES in ziptz/ziptz.py")
...
def main():
    ...
    print("wrote ziptz/ziptz.go and ziptz/ziptz.py")
